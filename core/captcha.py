...
class CAPTCHAManager:
    ...
    def _load_from_redis(self) -> None:
        """REM: CAPTCHA uses TTL keys — cannot enumerate via hgetall; skip bulk load on init.
        REM: Cross-worker lookup is done on-demand in verify_challenge via _load_challenge_from_redis."""
        pass

    def _load_challenge_from_redis(self, challenge_id: str) -> Optional["CAPTCHAChallenge"]:
        """REM: Look up a single challenge from Redis by ID (for cross-worker verification)."""
        try:
            from core.persistence import security_store
            data = security_store.get_record("captcha", challenge_id, use_ttl_key=True)
            if not data:
                return None
            # REM: Decrypt answer if it was encrypted on save
            answer = data.get("answer", "")
            if data.get("_answer_encrypted"):
                try:
                    from core.secure_storage import secure_storage
                    answer = secure_storage.decrypt_string(answer)
                except Exception:
                    pass  # REM: Use as-is if decryption fails — challenge will fail verification
            ...
            # REM: Cache locally to avoid repeated Redis lookups for this challenge
            self._challenges[challenge_id] = challenge
            return challenge
        except Exception as e:
            logger.warning(
                f"REM: Failed to load CAPTCHA challenge ::{challenge_id}:: from Redis: {e}_Thank_You_But_No"
            )
            return None
    ...
    def verify_challenge(self, challenge_id: str, user_answer: str) -> bool:
        ...
        challenge = self._challenges.get(challenge_id)
        if not challenge:
            # REM: Not in local memory — may have been generated by another worker; check Redis
            challenge = self._load_challenge_from_redis(challenge_id)
        if not challenge:
            logger.warning(
